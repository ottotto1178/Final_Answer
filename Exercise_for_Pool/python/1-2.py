...
for x in range(len(contents_links)):
  ...
  confirmation_mail_address = driver.find_elements(By.LINK_TEXT, 'お店に直接メールする')
  ...
  if len(confirmation_mail_address) >= 1:
    mail_address_tag = driver.find_element(By.LINK_TEXT, 'お店に直接メールする')
    mail_address_attribute = mail_address_tag.get_attribute('href')
    mail_address = re.sub('mailto:', '', mail_address_attribute)
  ...
  if len(confirmation_building) >=1:
    ...
    buildings.append('')
  confirmation_home_page = driver.find_elements(By.XPATH, '//*[@id="sv-site"]/li/a')
  if len(confirmation_home_page) >= 1:
    home_page_url = driver.find_element(By.XPATH, '//*[@id="sv-site"]/li/a')
    home_page_url.click()
    handle_array = driver.window_handles
    driver.switch_to.window(handle_array[-1])
    un_shorted_url = driver.current_url
    URL = un_shorted_url[:un_shorted_url.find('?')] #そのままではURLが長いので短縮する
    print(URL)
    URLs.append(URL)
  else:
    URLs.append('')
