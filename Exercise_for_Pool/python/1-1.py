...
for i in range(3): #50件取得のため3ページ分進める
  time.sleep(3) #待機時間3秒
  ...
  for x in range(len(restaurant_name_tags)):
    if len(restaurant_name_str) == 50:
      break
    if restaurant_name_tags[x].string == None: #PRの店舗を除外する
      continue
    else:
      restaurant_name_str += restaurant_name_tags[x]
...
for x in range(len(links)): #50回繰り返す
  ...
  URL = soup.find('a', class_='sv-of double') #sample.csvにおいてURLがサービス一覧のオフィシャルページだったためそこのURLを取得
